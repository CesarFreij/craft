...
export function SalesReturnsPage() {
  ...
  const openEditReturn = useCallback(async (returnId: string) => {
    try {
      ...
      for (const summary of returnList.filter((item) => item.salesInvoiceId === details.salesInvoiceId && item.id !== returnId)) {
        const current = await salesService.getReturnById(summary.id)
      ...
      console.error('OPEN EDIT SALES RETURN FAILED', error)
      setReturnError(getUserFriendlyErrorMessage(error, 'تعذر فتح بيانات مرتجع البيع للتعديل.'))
    }
  }, [])
            ...
            <Table sx={{ minWidth: 1000 }}>
              ...
              <TableBody>
                {returns.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7}>
                      <Typography sx={{ color: 'rgba(255, 255, 255, 0.72)', py: 2 }}>لا توجد مرتجعات بيع مسجلة حالياً.</Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  returns
